...
static OtrlUserState user_state;//这些多半是从C语言的OTR库里获取来的数据类型
...
static void
cb_inject_message(void *opdata, const char *accountname,//这是一条独有的OTR发送函数
    const char *protocol, const char *recipient, const char *message)
{
    cons_show("Function type:6.");
    char *id = message_send_chat_otr(recipient, message, FALSE);//现行jid,消息,false
    free(id);
}
...
otr_get_policy(const char *const recipient)
{
    ...
    cons_show(PROF_OTRPOLICY_MANUAL);
...
{
    cons_show("Function type:34.");
    otrl_message_free(message);
}

gboolean otr_fsend_start=false;
void
otr_fsend(const char *const recipient, char *path, const char *command)
{
    
/*
    //这里执行python脚本
    if (otr_fsend_start==false) {
        otr_fsend_start=true;pthread_t pyline;
        pthread_create(&pyline,NULL,(void *)&otr_fsend_python,(void *)&path);
        while (1) {
            char buf[SIZE]={0};   //接收读取的东西的盒子
            int ret = 0;          // ret表示read读取的字节数
            int retOpen =0;       //  是open的返回值   也就是后面要用的文件描述符
            char *a;
            retOpen = open("/home/waosp/Pictures/send.bin",O_RDONLY);
            if(retOpen<0)
            {
                return;
            }
            while(1){
                ...
                if(0==ret){
                    cons_show("file load finished.");
                    break;
                }else if(0>ret){
                    return;
                }
                a="读取到";
                strcat(a,(const char)ret);strcat(a,"个字节");
                cons_show(a);
            }
            close(retOpen);cons_show(buf);
        }
    } else {
        cons_show("You can not run two file send lines at same time.");
    }
    return;
*/
}
/*
void
otr_fsend_python(const char *path)
{
    Py_Initialize();
    PyRun_SimpleString("import sys");
    PyRun_SimpleString("sys.path.append('/home/waosp/Pictures')");
    PyObject *pmodule = PyImport_ImportModule("cutfile");
    PyObject *pfunc = PyObject_GetAttrString(pmodule, "file_send");
    PyObject_CallFunction(pfunc,path);
    Py_Finalize();
    otr_fsend_start=false;
}
*/
