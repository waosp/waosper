...
gboolean
cmd_otr_fsend(ProfWin *window, const char *const command, gchar **args)
{
#ifdef HAVE_LIBOTR
    if (connection_get_status() != JABBER_CONNECTED) {
        cons_show("You must be connected with an account to load OTR information.");
    ...
    if (window->type != WIN_CHAT) {
        win_println(window, THEME_DEFAULT, '-', "You must be in an OTR session to send a file.");
        return TRUE;
    }
    ...
    otr_fsend(chatwin->barejid, path, command);
    return TRUE;
#else
    cons_show("This version of Profanity has not been built with OTR support enabled");
    return TRUE;
...
cmd_otr_fsend(ProfWin *window, const char *const command, gchar **args)
{
    ...
    cons_show("ok");
    //otr_fsend(chatwin->barejid, path);
    return TRUE;
#else
    cons_show("This version of Profanity has not been built with OTR support enabled");
    return TRUE;
